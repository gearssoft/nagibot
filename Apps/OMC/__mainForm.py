...
class MainForm(QWidget, UI.mainForm.Ui_mainForm):
    ...
    def __init__(self, parent=None):
        ...
        if self.configMng.load_config() == True:
            ...
            print("ConfigManager: 현재 선택된 차량 인덱스:", self.current_unit_index)
            print("ConfigManager: 현재 선택된 서브 차량 인덱스:", self.current_unit_index_sub)

            if self.configMng.get_unit_enable(self.current_unit_index) is False:
                print(f"ConfigManager: 선택된 주 차량 {self.current_unit_index+1}호기가 비활성 상태입니다.")
            if self.configMng.get_unit_enable(self.current_unit_index_sub) is False:
                print(f"ConfigManager: 선택된 보조 차량 {self.current_unit_index_sub+1}호기가 비활성 상태입니다.")
        ...
        # 호기 표시
        self.txUnitNuberInfo.setText(f"{self.current_unit_index+1} 호기"); # 호기 표시를 비워둠
        ...
        if self.configMng.get_unit_enable(self.current_unit_index) is False:
            self.mainCamScreen_bmpLabel.setText(f"선택된 {self.current_unit_index+1}호기가 비활성 상태입니다.")     
            # self.labelSubCamera.setText(f"선택된 {self.current_unit_index+1}호기가 비활성 상태입니다.")
            # self.labelBottomRightScreen.setText(f"선택된 {self.current_unit_index+1}호기가 비활성 상태입니다.")

        else:
            # RTSP 스트림 설정
            self.rtsp_url = self.configMng.get_car_cam_url(car_idx=self.current_unit_index)
            self.rtsp_url_ir = self.configMng.get_car_cam_url_ir(car_idx=self.current_unit_index)
            
            print("RTSP URL:", self.rtsp_url)
            print("RTSP URL SubScreen:", self.rtsp_url_ir)
        ...
        if self.configMng.get_unit_enable(self.current_unit_index_sub) is False:
            self.labelSubCamera.setText(f"선택된 {self.current_unit_index_sub+1}호기가 비활성 상태입니다.")
