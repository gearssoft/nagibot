...
function setupRobotModeHandlers() {

    const inpRobotNumber = document.querySelector('#robot-operation-count input');
    const numberOFUnits = parseInt(inpRobotNumber.value, 3);
...
async function updateRobotModeStatus() {
...
async function main() {
    ...
    setupRobotModeHandlers(); //로봇 모드 핸들러 설정

    //로봇 모드 상태 갱신 주기적 실행
    function updateRobotModeStatus_loop() {
        updateRobotModeStatus().then(() => {
            setTimeout(updateRobotModeStatus_loop, 1000);
        });
    }
    updateRobotModeStatus_loop();
